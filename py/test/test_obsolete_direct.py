...
class TestObsoleteDirect:

    def test_should_direct_list_obsolete(self):
        setup = _obsolete_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-obsolete", "live" if setup["live"] else "unit")
        if _skip:
...
def _obsolete_direct_setup(mockres):
