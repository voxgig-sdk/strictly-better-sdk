    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STRICTLYBETTER_TEST_FUNCTIONAL_REPRINT_ENTID JSON to run live")
        client = setup["client"]
        ...
        functional_reprint_ref01_list_result, err = functional_reprint_ref01_ent.list(functional_reprint_ref01_match, None)
        assert err is None
...
def _functional_reprint_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STRICTLYBETTER_TEST_FUNCTIONAL_REPRINT_ENTID"))
