...
class TestObsoleteEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Obsolete(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Obsolete(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STRICTLY_BETTER_TEST_OBSOLETE_ENTID JSON to run live")
        client = setup["client"]
...
def _obsolete_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STRICTLY_BETTER_TEST_OBSOLETE_ENTID"))
