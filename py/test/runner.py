# StrictlyBetter SDK test runner
...
class StrictlyBetterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StrictlyBetterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StrictlyBetterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StrictlyBetterTestRunner.getenv("STRICTLY_BETTER_TEST_LIVE")
        override = StrictlyBetterTestRunner.getenv("STRICTLY_BETTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StrictlyBetterTestRunner.getenv(key)
        ...
        explain = StrictlyBetterTestRunner.getenv("STRICTLY_BETTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if StrictlyBetterTestRunner._test_control is not None:
            return StrictlyBetterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StrictlyBetterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StrictlyBetterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StrictlyBetterTestRunner.load_test_control()
...
def load_env_local():
    StrictlyBetterTestRunner.load_env_local()


def env_override(m):
    return StrictlyBetterTestRunner.env_override(m)


def entity_data(v):
    return StrictlyBetterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StrictlyBetterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StrictlyBetterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StrictlyBetterTestRunner.load_test_control()


def live_delay_ms():
    return StrictlyBetterTestRunner.live_delay_ms()
