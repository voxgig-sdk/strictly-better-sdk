...
class TestReprintDirect:

    def test_should_direct_list_reprint(self):
        setup = _reprint_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-reprint", "live" if setup["live"] else "unit")
        if _skip:
...
def _reprint_direct_setup(mockres):
