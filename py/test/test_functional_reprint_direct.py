...
class TestFunctionalReprintDirect:

    def test_should_direct_list_functional_reprint(self):
        setup = _functional_reprint_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-functional_reprint", "live" if setup["live"] else "unit")
        if _skip:
...
def _functional_reprint_direct_setup(mockres):
