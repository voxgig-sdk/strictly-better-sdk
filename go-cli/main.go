// Command StrictlyBetter-cli is a CLI + REPL for the StrictlyBetter SDK,
...
const prompt = "strictly-better"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "functional_reprint obsolete"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STRICTLY_BETTER_APIKEY (api key), STRICTLY_BETTER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STRICTLY_BETTER_APIKEY carries the API key and
	// STRICTLY_BETTER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STRICTLY_BETTER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STRICTLY_BETTER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStrictlyBetterSDK(opts)
