...
boru-driven command-line client **and** interactive REPL for the StrictlyBetter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/strictly-better-cli)
make build

# 2. See usage (words, entities, env vars)
./strictly-better-cli --help

# 3. Provide credentials once, via the environment
export STRICTLY_BETTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./strictly-better-cli list functional_reprint
./strictly-better-cli list obsolete

# 5. Override the API base URL for a single call
STRICTLY_BETTER_BASE=https://api.example.com ./strictly-better-cli list functional_reprint

# 6. No arguments -> interactive REPL
./strictly-better-cli
strictly-better> list functional_reprint
strictly-better> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/strictly-better-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/strictly-better-cli list functional_reprint
   ```
...
Run with no arguments to open a REPL (prompt `strictly-better>`). Each line is
evaluated as its own boru expression:

```text
$ ./strictly-better-cli
strictly-better> list functional_reprint
strictly-better> /help
strictly-better> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `functional_reprint`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/strictly-better-cli`. |
...
(`./strictly-better-cli <expr>`) and interactively (the REPL), and expressions compose the
