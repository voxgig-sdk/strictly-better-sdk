...
class ObsoleteDirectTest < Minitest::Test
  def test_direct_list_obsolete
    setup = obsolete_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-obsolete", setup[:live] ? "live" : "unit")
    if _should_skip
...
def obsolete_direct_setup(mockres)
