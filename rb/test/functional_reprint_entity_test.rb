...
class FunctionalReprintEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = StrictlyBetterSDK.test(seed, nil)
    seen = base.FunctionalReprint(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.FunctionalReprint(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STRICTLY_BETTER_TEST_FUNCTIONAL_REPRINT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    functional_reprint_ref01_list_result = functional_reprint_ref01_ent.list(functional_reprint_ref01_match, nil)
...
def functional_reprint_basic_setup(extra)
  ...
  entid_env_raw = ENV["STRICTLY_BETTER_TEST_FUNCTIONAL_REPRINT_ENTID"]
