...
class ReprintDirectTest < Minitest::Test
  def test_direct_list_reprint
    setup = reprint_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-reprint", setup[:live] ? "live" : "unit")
    if _should_skip
...
def reprint_direct_setup(mockres)
