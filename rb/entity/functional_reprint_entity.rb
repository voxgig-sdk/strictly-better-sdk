...
class FunctionalReprintEntity
  ...
  def make
    opts = @_entopts.dup
    FunctionalReprintEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StrictlyBetterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [FunctionalReprint, Hash] the current FunctionalReprint data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = StrictlyBetterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of FunctionalReprint fields)
  def match_get
  ...
  #
  # @param reqmatch [FunctionalReprintListMatch, Hash, nil] match filter (any subset of
  #   FunctionalReprint fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<FunctionalReprint>, Array] the matching FunctionalReprint items; raises StrictlyBetterError on failure
