...
module StrictlyBetterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StrictlyBetterResponse.new({})
    ...
    if fetch_err
      ...
      response = StrictlyBetterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StrictlyBetterResponse.new(fetched)
    else
