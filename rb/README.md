# StrictlyBetter Ruby SDK



The Ruby SDK for the StrictlyBetter API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.FunctionalReprint` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = StrictlyBetterSDK.test(testopts, sdkopts)
```
...
raise a `StrictlyBetterError` (a `StandardError` subclass), so wrap
...
```
rb/
├── StrictlyBetter_sdk.rb       -- Main SDK module
...
The main module (`StrictlyBetter_sdk`) exports the SDK class
...
functionalreprint.list()

# functionalreprint.data_get now returns the functionalreprint data from the last list
# functionalreprint.match_get returns the last match criteria
```
