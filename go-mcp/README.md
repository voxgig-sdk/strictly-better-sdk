...
[MCP](https://modelcontextprotocol.io) server exposing the StrictlyBetter SDK as
two agent tools — `strictly-better_list` and `strictly-better_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/strictly-better-mcp)
make build

# 2. Provide credentials via the environment
export STRICTLY_BETTER_APIKEY=sk_live_xxx
...
```jsonc
// strictly-better_list: first page of records
{ "entity": "functional_reprint" }
{ "entity": "functional_reprint", "query": { } }

// strictly-better_load: one record by id
{ "entity": "functional_reprint", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/strictly-better-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user strictly-better \
     -- "$PWD"/dist/*/strictly-better-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `strictly-better_list` and `strictly-better_load` tools now appear
   in new sessions. Ask the agent to *"list functional_reprint using strictly-better"*
   and it calls `strictly-better_list` with `{"entity":"functional_reprint"}`.
...
```sh
export STRICTLY_BETTER_APIKEY=sk_live_xxx            # API key
export STRICTLY_BETTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `strictly-better_list` tool
...
### Call the `strictly-better_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `strictly-better_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `strictly-better_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STRICTLY_BETTER_APIKEY` | API key sent with every request. |
| `STRICTLY_BETTER_BASE` | Optional override of the API base URL. |
