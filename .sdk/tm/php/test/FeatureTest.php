...
// StrictlyBetter SDK feature test
//
...
class FtCtrl extends StrictlyBetterControl
{
...
class FtClient
{
    ...
    private StrictlyBetterUtility $utility;

    public function __construct(StrictlyBetterUtility $utility, array $options)
    {
    ...
    public function get_utility(): StrictlyBetterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StrictlyBetterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StrictlyBetterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StrictlyBetterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StrictlyBetterConfig::make_feature($name);
    ...
    private static function buildUrl(StrictlyBetterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StrictlyBetterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StrictlyBetterResponse($fetched);
        }
    ...
    private function fail(StrictlyBetterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StrictlyBetterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
