...
//     (\`new StrictlyBetterSDK(...)\` / \`StrictlyBetterSDK.test(...)\` become
//     \`StrictlyBetterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'StrictlyBetterSDK'
...
const TEST_SEED = {"entity":{"functional_reprint":{"test01":{"id":"test01"}},"obsolete":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StrictlyBetterSDK(...)\` and any
// \`StrictlyBetterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StrictlyBetterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StrictlyBetterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
