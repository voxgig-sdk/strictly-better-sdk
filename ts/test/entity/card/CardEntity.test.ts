...
describe('CardEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set STRICTLY_BETTER_TEST_CARD_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['STRICTLY_BETTER_TEST_CARD_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'STRICTLY_BETTER_TEST_CARD_ENTID': idmap,
    'STRICTLY_BETTER_TEST_LIVE': 'FALSE',
    'STRICTLY_BETTER_TEST_EXPLAIN': 'FALSE',
    'STRICTLY_BETTER_APIKEY': 'NONE',
  })

  idmap = env['STRICTLY_BETTER_TEST_CARD_ENTID']
