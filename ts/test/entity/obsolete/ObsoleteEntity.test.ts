...
describe('ObsoleteEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'obsolete.' + op, live)) return
    }
    ...
    let obsolete_ref01_data = Object.values(setup.data.existing.obsolete)[0] as any

    // LIST
    const obsolete_ref01_ent = client.Obsolete()
    const obsolete_ref01_match: any = {}

    const obsolete_ref01_list = (await obsolete_ref01_ent.list(obsolete_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/obsolete/ObsoleteTestData.json')
  ...
  const idmapEnvVal = process.env['STRICTLY_BETTER_TEST_OBSOLETE_ENTID']
  ...
  idmap = env['STRICTLY_BETTER_TEST_OBSOLETE_ENTID']
