...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.STRICTLYBETTER_TEST_LIVE ||
    'TRUE' === process.env.STRICTLYBETTER_TEST_OVERRIDE
  ) {
  ...
  m.STRICTLYBETTER_TEST_EXPLAIN = process.env.STRICTLYBETTER_TEST_EXPLAIN || m.STRICTLYBETTER_TEST_EXPLAIN
