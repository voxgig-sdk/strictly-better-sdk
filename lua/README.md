...
The Lua SDK for the StrictlyBetter API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:FunctionalReprint(nil):list(nil, nil)
...
local result, err = client:StrictlyBetter(nil):load(
  { id = "test01" }, nil
)
