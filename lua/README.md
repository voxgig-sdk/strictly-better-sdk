# StrictlyBetter Lua SDK



The Lua SDK for the StrictlyBetter API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:FunctionalReprint()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/strictly-better-sdk/releases)),
...
```
lua/
├── strictly-better_sdk.lua    -- Main SDK module
...
The main module (`strictly-better_sdk`) exports the SDK constructor
