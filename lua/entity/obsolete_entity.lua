...
local ObsoleteEntity = {}
ObsoleteEntity.__index = ObsoleteEntity


function ObsoleteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ObsoleteEntity)
  self._name = "obsolete"
  self._client = client
...
function ObsoleteEntity:get_name()
  return self._name
end


function ObsoleteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ObsoleteEntity.new(self._client, opts)
end
...
function ObsoleteEntity:mark_deleted()
  self._deleted = true
end


function ObsoleteEntity:deleted()
  return true == self._deleted
end


function ObsoleteEntity:data_set(args)
  if args ~= nil then
...
function ObsoleteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ObsoleteEntity:match_set(args)
  if args ~= nil then
...
function ObsoleteEntity:match_get()
...
function ObsoleteEntity:stream(action, args, callopts)
...
function ObsoleteEntity:_run_op(ctx, post_done)
  local utility = self._utility
