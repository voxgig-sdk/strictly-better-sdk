...
func TestCardEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STRICTLYBETTER_TEST_CARD_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func cardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STRICTLYBETTER_TEST_CARD_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"STRICTLYBETTER_TEST_CARD_ENTID": idmap,
		"STRICTLYBETTER_TEST_LIVE":      "FALSE",
		"STRICTLYBETTER_TEST_EXPLAIN":   "FALSE",
		"STRICTLYBETTER_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["STRICTLYBETTER_TEST_CARD_ENTID"])
