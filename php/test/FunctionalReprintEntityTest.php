...
class FunctionalReprintEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = StrictlyBetterSDK::test(null, null);
        $ent = $testsdk->FunctionalReprint(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->FunctionalReprint(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = StrictlyBetterConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = StrictlyBetterSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->FunctionalReprint(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = functional_reprint_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "functional_reprint." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STRICTLY_BETTER_TEST_FUNCTIONAL_REPRINT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $functional_reprint_ref01_data = null;
        if (count($functional_reprint_ref01_data_raw) > 0) {
...
function functional_reprint_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/functional_reprint/FunctionalReprintTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["functional_reprint01", "functional_reprint02", "functional_reprint03"] as $k) {
    ...
    $entid_env_raw = getenv("STRICTLY_BETTER_TEST_FUNCTIONAL_REPRINT_ENTID");
