...
// StrictlyBetter SDK FunctionalReprint entity
...
class FunctionalReprintEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FunctionalReprintEntity($this->_client, $opts);
    }

    /**
     * @param FunctionalReprint|array $args FunctionalReprint data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = StrictlyBetterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return FunctionalReprint|array The current FunctionalReprint data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of FunctionalReprint fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = StrictlyBetterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of FunctionalReprint fields).
     */
    ...
    /**
     * List FunctionalReprint items matching the given filter.
     *
     * @param FunctionalReprintListMatch|array|null $reqmatch Match filter (any subset
     *   of FunctionalReprint fields) as an assoc-array; FunctionalReprintListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FunctionalReprint[]|array A list of FunctionalReprint items as assoc-arrays at
     *   the SDK boundary; throws StrictlyBetterError on failure (item-5 convention).
     */
