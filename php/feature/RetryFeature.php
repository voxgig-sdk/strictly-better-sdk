...
// StrictlyBetter SDK retry feature
...
class StrictlyBetterRetryFeature extends StrictlyBetterBaseFeature
{
    ...
    public function init(StrictlyBetterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (StrictlyBetterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(StrictlyBetterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(StrictlyBetterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
